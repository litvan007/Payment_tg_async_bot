...
async def send_welcome(message):
    ...
    if user_id in curr_users:
        await bot.send_message(
            599202079,
            text=f'Пользователь {message.from_user.username if message.from_user.username is not None else "Empty"} <b> ПРОДОЛЖИЛ </b> работу с ботом',
            parse_mode="HTML",
        )
    else:
        await bot.send_message(
            599202079,
            text=f'Пользователь {message.from_user.username if message.from_user.username is not None else "Empty"} <b> Начал </b> работу с ботом',
            parse_mode="HTML",
        )
        ...
        if value == "Not having db files":
            await bot.send_message(
                599202079,
                text=f"Были удалены все БД. Пользователь {message.from_user.username} не смог добавиться",
                parse_mode="HTML",
            )
            sys.exit()
    ...
    btn1 = types.InlineKeyboardButton(
        "Получить доступ", callback_data="to_buying_enter"
    )
    btn2 = types.InlineKeyboardButton(
        "Прослушать промо книги", callback_data="get_promo"
    )
    markup.add(btn1)
    markup.add(btn2)
    ...
    await bot.send_message(
        message.chat.id,
        text="📚 Добро пожаловать в бот аудиокниги «Город поющих мостов». Здесь можно прослушать отрывок из произведения, а также получить доступ к полной версии книги, в которой ты найдёшь захватывающие истории, загадочных персонажей и магические приключения. Погрузись в мир Сказки вместе с нами. 🌟🎧",
        reply_markup=markup,
    )
...
async def send_welcome(message):
    ...
    if users_or_value == "Not having db files":
        await bot.send_message(
            599202079,
            text=f"Были удалены все БД. Пользователь {message.from_user.username} не смог перейти к оплате",
            parse_mode="HTML",
        )
        sys.exit()
    ...
    if payments_info[user_id] == True:
        await bot.send_message(
            message.chat.id,
            text=f"Оплата уже была успешно совершена. Ссылка на канал: {config['other']['link_for_joining']}",
        )
    else:
        await bot.send_message(message.chat.id, text=f"Оплата еще не была произведена.")
        prices = [
            types.LabeledPrice(label="Город поющих мостов", amount=100000)
        ]  # amount в копейках
...
async def send_welcome(message):
    await bot.send_message(
        message.chat.id,
        text="Привет! Бот в первую очередь необходим для более удобного получения доступа к каналу. По всем вопросам обращайтесь ко мне.",
    )


@bot.callback_query_handler(func=lambda call: call.data == "get_promo")
async def getting_promo_mode(
    call,
):  # проверить, что человек внутри канала + рекомендовать донат в ином случае
    async with aiofiles.open(config["other"]["promo_file_path"], mode="rb") as f:
        audio_data = await f.read()
        await bot.send_audio(
            call.message.chat.id, audio_data, caption="Промо аудиокниги ⏯️"
        )


@bot.callback_query_handler(func=lambda call: call.data == "to_buying_enter")
async def purchasing_mode(
    call,
):  # проверить, что человек внутри канала + рекомендовать донат в ином случае
    ...
    if users_or_value == "Not having db files":
        await bot.send_message(
            599202079,
            text=f"Были удалены все БД. Пользователь {call.from_user.username} не смог перейти к оплате",
            parse_mode="HTML",
        )
        sys.exit()
    ...
    if payments_info[user_id] == False:
        ...
        await bot.send_message(
            call.message.chat.id, text="Если вы хотите задонатить, то свяжитесь со мной"
        )
...
async def got_payment(message):
    ...
    if users_or_value == "Not having db files":
        await bot.send_message(
            599202079,
            text=f"Были удалены все БД. Пользователь {message.from_user.username} после оплаты не получил сообщения об оплате. Пусть проверит списание",
            parse_mode="HTML",
        )
        sys.exit()
    ...
    if payments_info[user_id] == False:
        await bot.send_message(
            message.chat.id,
            f"""✅ Ваша подписка оформлена! 🎧
[🔗 Нажмите здесь, чтобы перейти в канал!]({config["other"]["link_for_joining"]})""",
            parse_mode="Markdown",
        )
...
async def allow_request(message: types.ChatJoinRequest):
    ...
    if users_or_value == "Not having db files":
        await bot.send_message(
            599202079,
            text=f"Были удалены все БД. Пользователь {message.from_user.username} после возможной оплаты не смог добавить к каналу. Необходимо проверить платеж и добавить человека",
            parse_mode="HTML",
        )
        sys.exit()
...
Вы успешно подключились к нашему эксклюзивному каналу, где звучат мелодии слов и волшебство сюжета. Приготовьтесь к путешествию по страницам этой захватывающей истории. Приятного прослушивания! 📖🎧""",
        ...
        await bot.send_message(
            599202079,
            f"{message.from_user.username if message.from_user.username is not None else 'Empty'}_{user_id} оплатил доступ",
        )
...
if __name__ == "__main__":
    ...
    if not os.path.isfile(path_to_db) and not os.path.isfile(
        config["db"]["dest_db_path"]
    ):
